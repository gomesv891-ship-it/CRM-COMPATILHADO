...
export const MetasScreen: React.FC<MetasScreenProps> = ({
  ...
  const [editVanessaValue, setEditVanessaValue] = useState<string>('80000');
  const [editJhessicaValue, setEditJhessicaValue] = useState<string>('70000');
  ...
  const responsibleCounts = useMemo(() => {
    ...
    return {
      todos: vendas.length,
      eder: vendas.filter((v) => isRecordOfResponsible(v, 'Éder')).length,
      vanessa: vendas.filter((v) => isRecordOfResponsible(v, 'Vanessa')).length,
      jhessica: vendas.filter((v) => isRecordOfResponsible(v, 'Jhessica')).length,
  ...
  const handleOpenEditMeta = () => {
    ...
    setEditVanessaValue((individualMetas.vanessa || 80000).toString());
    setEditJhessicaValue((individualMetas.jhessica || 70000).toString());
  ...
  // REGRA: META TODOS = META ÉDER + META VANESSA + META JHESSICA
  // Calculada automaticamente com base nos valores individuais cadastrados.
  // Se qualquer um dos três usuários alterar sua meta individual, a meta "Todos" é atualizada automaticamente.
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 self-start md:self-center">
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-2xl bg-gradient-to-r from-[#0052cc] to-[#0072ff] hover:from-blue-700 hover:to-blue-600 text-white font-bold text-xs sm:text-sm shadow-md hover:shadow-lg transition-all flex items-center gap-2 cursor-pointer"
          >
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 w-full max-w-md space-y-4 animate-in zoom-in-95 duration-150 text-slate-800">
            ...
            {isDirector && responsibleTab === 'Todos' ? (
              <div className="space-y-3.5">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">
                    Meta Jéssica Camargo (Consultora)
                  </label>
                  <div className="relative">
                    <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">R$</span>
                    <input
                ...
                <div className="p-3.5 rounded-2xl bg-purple-50 border border-purple-200">
                  ...
                  <span className="text-[10px] text-purple-700 block mt-0.5">
                    Éder ({formatCurrency(parseFloat(editEderValue) || 0)}) + Vanessa ({formatCurrency(parseFloat(editVanessaValue) || 0)}) + Jéssica ({formatCurrency(parseFloat(editJhessicaValue) || 0)})
                  </span>
                </div>
              </div>
            ) : (
