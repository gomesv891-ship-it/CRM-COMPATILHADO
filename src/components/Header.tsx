...
export const Header: React.FC<HeaderProps> = ({
  ...
  const handleSelectSearchResult = (result: GlobalSearchResult) => {
    setIsSearchOpen(false);
    setSearchQuery('');
    if (result.clientRecord && onSelectClient) {
  ...
  return (
    <header className="relative w-full h-32 sm:h-36 overflow-visible bg-[#0a1428] select-none z-30">
      ...
      {/* Content Container */}
      <div className="relative z-10 h-full w-full px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-14 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 sm:gap-3.5 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1 py-1">
            <h1 className="text-lg sm:text-2xl lg:text-[26px] font-bold text-white tracking-tight leading-tight drop-shadow-xs truncate">
              Olá, {firstName}!
            </h1>
            ...
            {/* Frase Bíblica do Dia (Canônica e Validada) - Exibição Completa em Linha Única */}
            <div className="mt-1.5 flex items-center max-w-full overflow-x-auto no-scrollbar">
              <div
                className="inline-flex items-center flex-nowrap whitespace-nowrap gap-2 px-2.5 sm:px-3 py-1 rounded-xl bg-white/10 hover:bg-white/15 border border-white/15 backdrop-blur-md text-white transition-colors select-none"
                title={`Versículo do dia: "${verseOfTheDay.text}" — ${verseOfTheDay.reference}`}
              >
                <BookOpen className="w-3.5 h-3.5 text-amber-300 shrink-0" />
                <span className="text-[11px] sm:text-xs font-normal text-slate-100 italic whitespace-nowrap">
        ...
        {/* Right Side: Sino de Notificações + BUSCA GLOBAL + Perfil da Usuária */}
        <div className="flex items-center gap-2 sm:gap-2.5 lg:gap-3 flex-shrink-0">
          {/* ==========================================
              3. SINO DE NOTIFICAÇÕES (com abas de categorias)
              Todas | Tarefas | Boletos | Follow-up | Meta | Notas | Pendências
             ========================================== */}
