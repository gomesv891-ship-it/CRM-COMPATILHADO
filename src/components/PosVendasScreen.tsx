...
export const PosVendasScreen: React.FC<PosVendasScreenProps> = ({
  ...
  useEffect(() => {
    try {
      ...
      if (records.length > 0) {
        saveWholeCollectionToSupabase('fenix_pos_vendas_db', records).catch(() => {});
      }
    } catch {
      // ignore
    }
  ...
  // Scoped records based on director tab or logged in common user
  const userRecords = useMemo(() => {
    if (isDirector) {
      if (responsibleTab === 'Todos') return records;
      return records.filter((r) => isRecordOfResponsible(r, responsibleTab));
    }
    // Usuário comum visualiza somente seus próprios registros
    return records.filter((r) => isRecordOfResponsible(r, currentUserName || ''));
  }, [records, isDirector, responsibleTab, currentUserName]);
  ...
  // Change status directly in row
  const handleStatusChange = (id: string, newStatus: PosVendaStatus) => {
    setRecords((prev) => {
      ...
      try {
        ...
        saveWholeCollectionToSupabase('fenix_pos_vendas_db', updated).catch(() => {});
      } catch {
        // ignore
      }
      return updated;
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-10 py-6 space-y-6 pb-12">
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="h-10 px-5 rounded-xl bg-[#1D4ED8] hover:bg-blue-700 text-white font-bold text-xs sm:text-sm flex items-center gap-2 shadow-sm transition-all cursor-pointer self-start sm:self-center active:scale-95"
          >
      ...
      {isDirector && (
        <ResponsibleFilterTabs
          ...
          label="Pós-Vendas por Responsável"
        />
      )}
