...
export const PendenciasScreen: React.FC<PendenciasScreenProps> = ({
  ...
  // REGRA: O usuário que CRIOU continua vendo sempre (não desaparece ao atribuir).
  // O usuário que RECEBEU a atribuição também vê.
  const userScopedPendencias = useMemo(() => {
    const currentUserId = getUserIdByName(currentUserName);
    const ederUserId = getUserIdByName('Eder Perez');

    return pendencias.filter((item) => {
      const itemCreatorId = (item as any).creatorId || item.criadoPorId;
      const itemRespId = item.responsavelId || (item as any).atribuidoAId;

      if (isDirector) {
        if (directorScopeFilter === 'Atribuidas') {
          const isAssigned =
            areUsersEqualOrRelated(item.atribuidoA, 'Eder') ||
            (Boolean(ederUserId && itemRespId) && itemRespId === ederUserId);
          const isMine =
            areUsersEqualOrRelated(item.criadoPor, 'Eder') ||
            (Boolean(ederUserId && itemCreatorId) && itemCreatorId === ederUserId);
      ...
      // 1. Quem CRIOU vê sempre (mesmo se atribuir para outra pessoa)
      const isMineById = Boolean(currentUserId && itemCreatorId && currentUserId === itemCreatorId);
      const isMineByName = areUsersEqualOrRelated(item.criadoPor, currentUserName);
      const isMine = isMineById || isMineByName;

      // 2. Quem RECEBEU a atribuição vê sempre
      const isAssignedToMeById = Boolean(currentUserId && itemRespId && currentUserId === itemRespId);
      const isAssignedToMeByName =
        areUsersEqualOrRelated(item.atribuidoA, currentUserName) ||
        areUsersEqualOrRelated((item as any).responsavel, currentUserName);
      const isAssignedToMe = isAssignedToMeById || isAssignedToMeByName;

      // 3. Pendências atribuídas ao Diretor continuam visíveis para acompanhamento por quem criou
      const isAssignedToDirector =
        areUsersEqualOrRelated(item.atribuidoA, 'Eder') ||
        (Boolean(ederUserId && itemRespId) && itemRespId === ederUserId);

      // 4. Pendências gerais ou sem atribuição
      const isGeneral =
        !item.atribuidoA ||
        item.atribuidoA === 'Geral' ||
        item.atribuidoA === 'Todos' ||
        item.atribuidoA === 'Equipe' ||
        (item as any).responsavel === 'Geral' ||
        (item as any).responsavel === 'Todos';

      // Fallback: se não tiver criador ou responsável especificado, visível a todos
      const isUnclaimed = !itemCreatorId && !item.criadoPor && !itemRespId && !item.atribuidoA;

      return isMine || isAssignedToMe || isAssignedToDirector || isGeneral || isUnclaimed;
    });
  ...
  const countAtribuidasAoDiretor = useMemo(() => {
    return userScopedPendencias.filter(
      (p) => areUsersEqualOrRelated(p.atribuidoA, 'Eder') && p.status !== 'Resolvida'
    ).length;
  }, [userScopedPendencias]);
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-10 py-6 space-y-6 text-slate-800 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#1D4ED8] hover:bg-[#1e40af] text-white text-sm font-semibold shadow-sm hover:shadow transition-all cursor-pointer flex-shrink-0 self-start sm:self-center"
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <p className="text-sm font-bold text-slate-900">Visão Geral da Diretoria — Eder Perez</p>
                <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[10px] font-bold border border-amber-200">
                  Acesso Total
                </span>
              </div>
              <p className="text-xs text-slate-600 mt-0.5">
                Acompanhe as pendências atribuídas a você por <strong>Vanessa Gomes</strong> e <strong>Jhessica Camargo</strong> com notificações em tempo real.
              </p>
            </div>
          </div>
