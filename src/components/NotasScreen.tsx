...
export const NotasScreen: React.FC<NotasScreenProps> = ({
  ...
  const handleDeleteNote = async (id: string, title: string) => {
    if (window.confirm(`Tem certeza que deseja excluir a anotação "${title}"?`)) {
  ...
  const displayedNotes = useMemo(() => {
    ...
    // Filter by Tab:
    // "Todas": Minhas notas + notas compartilhadas comigo (ou visão geral para Diretor)
    ...
    if (activeTab === 'todas') {
      list = list.filter((n) => isAuthor(n) || isSharedWithMe(n) || isDirector);
      ...
      list = list.filter((n) => n.isShared && (isAuthor(n) || isSharedWithMe(n) || isDirector));
    }
  ...
  const tabCounts = useMemo(() => {
    ...
    const visibleNotes = list.filter((n) => isAuthor(n) || isSharedWithMe(n) || isDirector);

    return {
      todas: visibleNotes.length,
      minhas: list.filter((n) => isAuthor(n)).length,
      comigo: list.filter((n) => !isAuthor(n) && isSharedWithMe(n)).length,
      compartilhadas: list.filter((n) => n.isShared && (isAuthor(n) || isSharedWithMe(n) || isDirector)).length,
    };
  }, [notes, currentUserName, isDirector]);
