...
export const OrcamentoScreen: React.FC<OrcamentoScreenProps> = ({
  ...
  const persistOrcamentoToDatabase = async (silent = false): Promise<{ success: boolean; data?: SavedOrcamento; error?: string }> => {
    const orcId = initialOrcamento?.id || `orc_${Date.now()}`;
    const now = new Date();
  ...
  // Gerar PDF / Imprimir Oficial (Salva no banco automaticamente)
  const handlePrintPdf = () => {
    persistOrcamentoToDatabase();
    showToast('✓ Orçamento salvo automaticamente no banco de dados!');
    window.print();
  };

  // Baixar Orçamento como PDF Oficial (Salva no banco automaticamente)
  const handleDownloadPdf = async () => {
    ...
    try {
      // Salva no banco de dados automaticamente ao clicar em Gerar PDF
      persistOrcamentoToDatabase();
      ...
      pdf.save(filename);
      showToast('✓ PDF oficial gerado e orçamento salvo no banco de dados!');
    } catch (err) {
  ...
  // Baixar como Imagem (PNG) em Alta Resolução (Salva no banco automaticamente)
  const handleDownloadImage = async () => {
    ...
    try {
      // Salva no banco de dados automaticamente ao clicar em Baixar Imagem
      persistOrcamentoToDatabase();
      ...
      showToast('✓ Imagem baixada e orçamento salvo no banco de dados!');
    } catch (err) {
  ...
  const handleSendWhatsApp = () => {
    ...
    showToast('Mensagem do orçamento preparada para o WhatsApp!');
  };
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-10 py-6 sm:py-8 space-y-8">
      ...
      <section className="no-print bg-white rounded-2xl border border-slate-200/90 shadow-sm p-5 sm:p-7 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="w-full h-11 rounded-xl border border-slate-200 bg-slate-50 px-3.5 flex items-center text-xs sm:text-sm font-semibold text-slate-800">
              {clientContact}
            </div>
          </div>
        </div>
      ...
      <section className="no-print bg-white rounded-2xl border border-slate-200/90 shadow-sm p-5 sm:p-7 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {/* Consultora Comercial (preenchida automaticamente com usuária logada) */}
          <div className="space-y-1.5">
