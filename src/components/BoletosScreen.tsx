...
export const BoletosScreen: React.FC<BoletosScreenProps> = ({
  ...
  // Diretor oversight filter: 'Todos' | 'Meus' | 'Vanessa Gomes' | 'Jhessica Camargo'
  // Filtro de autor do Diretor (Padrão: Destinados a ele ou criados por ele)
  const [directorAuthorFilter, setDirectorAuthorFilter] = useState<string>('Destinados');
  ...
  // Cálculo automático das parcelas (número, valor e vencimento)
  const calculatedParcelas: BoletoParcela[] = useMemo(() => {
    const count = Math.max(1, Math.min(60, Number(formQtdParcelas) || 1));
    const rawVal = parseFloat(formValorTotal.replace(/\./g, '').replace(',', '.')) || 0;
    const interval = Math.max(1, Number(formIntervaloDias) || 30);
    const baseDueDate = formPrimeiraParcela || getDynamicTodayDate();

    const list: BoletoParcela[] = [];
    const baseParcela = rawVal > 0 ? Math.floor((rawVal / count) * 100) / 100 : 0;
    const remainder = rawVal > 0 ? Math.round((rawVal - baseParcela * count) * 100) / 100 : 0;

    for (let i = 0; i < count; i++) {
      const num = `${i + 1}/${count}`;
      const valor = i === count - 1 ? Number((baseParcela + remainder).toFixed(2)) : baseParcela;
      const dueDate = i === 0 ? baseDueDate : addDaysToDateStr(baseDueDate, i * interval);
    ...
    return list;
  }, [formQtdParcelas, formValorTotal, formIntervaloDias, formPrimeiraParcela]);
  ...
  // A atribuição define o RESPONSÁVEL pelo registro, mas não transfere a propriedade exclusiva nem remove o registro da visão do usuário que o cadastrou."
  const userScopedBoletos = useMemo(() => {
    if (isDirector) {
      if (directorAuthorFilter === 'Destinados') {
        return currentMonthBoletos.filter((b) => {
          const isDestinedToMe =
            areUsersEqualOrRelated(b.destinadoA, 'Eder') ||
            areUsersEqualOrRelated(b.responsavel, 'Eder');
          const isCreatedByMe = areUsersEqualOrRelated(b.criadoPor, 'Eder');
          return isDestinedToMe || isCreatedByMe;
        });
      }
      if (directorAuthorFilter === 'Todos') {
        return currentMonthBoletos;
      }
      if (directorAuthorFilter === 'Meus') {
        return currentMonthBoletos.filter((b) => areUsersEqualOrRelated(b.criadoPor, 'Eder'));
      }
      // Filtro por pessoa específica: mostra registros criados pela pessoa OU atribuídos a ela
      const target = directorAuthorFilter.toLowerCase().trim();
      return currentMonthBoletos.filter((b) => {
        return (
          areUsersEqualOrRelated(b.criadoPor, target) ||
          areUsersEqualOrRelated(b.destinadoA, target) ||
          areUsersEqualOrRelated(b.responsavel, target)
        );
      });
    }

    // Se for consultora/colaboradora (Vanessa ou Jéssica):
    // 1. Quem cadastrou NUNCA perde a visão do boleto (mesmo atribuído a Éder Perez ou outro usuário)
    // 2. Boletos destinados/atribuídos a ela aparecem no seu sistema
    // 3. Boletos gerais também são visíveis
    return currentMonthBoletos.filter((b) => {
      if (!b.criadoPor && !b.destinadoA) return true; // legados são mostrados
      ...
      const isGeneral =
        ...
        b.responsavel === 'Geral' ||
        b.responsavel === 'Todos';
      return isCreator || isResponsible || isGeneral;
    });
  }, [currentMonthBoletos, isDirector, directorAuthorFilter, currentUserName]);
    ...
    try {
      if (editingBoleto) {
        const updatedBoleto: BoletoItem = {
          ...
          intervaloDias: Number(formIntervaloDias) || 30,
          parcelas: calculatedParcelas,
        };
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-10 py-6 space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="h-10 px-5 rounded-xl bg-[#0052CC] hover:bg-blue-700 text-white font-bold text-xs sm:text-sm flex items-center gap-2 shadow-sm transition-all cursor-pointer self-start sm:self-center active:scale-95"
          >
      ...
      <div className="space-y-2.5">
        ...
        {/* VISÃO ESPECÍFICA DO DIRETOR EDER PEREZ */}
        {isDirector && (
          <div className="p-3 bg-gradient-to-r from-blue-50/90 to-indigo-50/70 border border-blue-200/80 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-[#0052cc] uppercase tracking-wide flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-[#0052cc]" />
                Painel do Diretor (Visualização Geral da Equipe):
              </span>
            </div>
            <div className="flex items-center gap-1.5 flex-wrap">
              {[
                { id: 'Destinados', label: 'Destinados a Mim / Meus' },
                { id: 'Todos', label: 'Todos os Boletos' },
                { id: 'Meus', label: 'Cadastrados por Mim' },
                { id: 'Vanessa Gomes', label: 'Vanessa Gomes' },
                { id: 'Jhessica Camargo', label: 'Jhessica Camargo' },
              ].map((pill) => {
                const isSelected = directorAuthorFilter === pill.id;
                return (
                  <button
                    key={pill.id}
                    type="button"
                    onClick={() => setDirectorAuthorFilter(pill.id)}
          ...
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200/80 text-[11px] font-semibold text-slate-600 uppercase tracking-wider select-none">
                <th className="py-4 px-6 w-28">Pedido</th>
                <th className="py-4 px-6">Cliente</th>
                <th className="py-4 px-6 w-40">Cadastrado em</th>
                <th className="py-4 px-6 w-48">Parcelas / Vencimento</th>
                <th className="py-4 px-6 w-44 text-right sm:text-left">Status</th>
                <th className="py-4 px-6 w-44">Responsável</th>
            ...
            <tbody className="divide-y divide-slate-100 text-xs sm:text-sm">
              {displayedBoletos.length === 0 ? (
                  ...
                  <td colSpan={7} className="py-12 px-4 text-center">
                    <div className="max-w-xs mx-auto space-y-2">
                      ...
                      <p className="text-xs text-slate-400">
                        ...
                        {isDirector && directorAuthorFilter !== 'Todos' ? ` pelo filtro "${directorAuthorFilter}"` : ''}.
                      </p>
                      <button
                        type="button"
                        onClick={handleOpenModal}
                ...
                displayedBoletos.map((item, idx) => {
                  ...
                  // Verifica se a primeira parcela está no mês seguinte ao cadastro
                  const cadDate = item.dataCadastro || item.createdAt;
                  const cadMonth = cadDate ? parseInt(cadDate.split('-')[1], 10) : null;
                  const dueMonth = item.firstDueDate ? parseInt(item.firstDueDate.split('-')[1], 10) : null;
                  const isNextMonthInstallment = cadMonth !== null && dueMonth !== null && dueMonth !== cadMonth;
                  ...
                  return (
                    ...
                    >
                      {/* Coluna 1: PEDIDO */}
                      <td className="py-4 px-6 font-bold text-[#071a52] text-sm">
                      ...
                      {/* Coluna 4: PARCELAS / VENCIMENTO */}
                      <td className="py-4 px-6 text-slate-600 font-medium">
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5">
                            <Clock className="w-3.5 h-3.5 text-slate-400" />
                            <span className="font-semibold text-slate-800">{formatDateBR(item.firstDueDate)}</span>
                          </div>
                          {((item.qtdParcelas && item.qtdParcelas > 1) || (item.parcelas && item.parcelas.length > 1)) ? (
                            <div className="flex items-center gap-1 flex-wrap">
                              <span className="inline-block text-[10px] font-bold text-blue-700 bg-blue-50 border border-blue-200 px-1.5 py-0.5 rounded">
                                {item.qtdParcelas || item.parcelas?.length}x ({item.intervaloDias || 30} dias)
                              </span>
                              {item.valorTotal && (
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl max-w-md w-full border border-slate-200 shadow-2xl overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSaveModal} className="p-6 space-y-4 text-xs sm:text-sm max-h-[80vh] overflow-y-auto custom-scrollbar">
              ...
              {/* Campo 4: Primeira Parcela */}
              <div className="space-y-1.5">
                <label className="font-bold text-slate-700">
                  Data da 1ª Parcela <span className="text-rose-500">*</span>
                </label>
                <input
                  type="date"
                ...
                />
                {formDataCadastro && formPrimeiraParcela && formDataCadastro.slice(0, 7) !== formPrimeiraParcela.slice(0, 7) && (
                  <div className="p-2.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-xs flex items-start gap-2">
                    ...
                    <span>
                      <strong>Aviso:</strong> A 1ª parcela vence em <strong>{formatDateBR(formPrimeiraParcela)}</strong> (mês seguinte), mas o boleto permanecerá registrado e visível no mês de cadastro (<strong>{formatDateBR(formDataCadastro).slice(3)}</strong>).
                    </span>
                  </div>
                )}
                {formErrors.primeiraParcela && (
              ...
              {/* REGRAS 20: QUANTIDADE DE PARCELAS, VALOR E INTERVALO */}
              <div className="p-3.5 bg-slate-50/80 rounded-xl border border-slate-200 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                    <Calendar className="w-4 h-4 text-[#0052CC]" />
                    <span>Configuração de Parcelas e Vencimento Automático</span>
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full h-9 px-3 rounded-lg border border-slate-200 bg-white text-xs font-medium text-slate-800 focus:border-[#0052CC] outline-none"
                    />
                  </div>

                  {/* Quantidade de Parcelas */}
                  <div className="space-y-1">
                    <label className="text-[11px] font-bold text-slate-700">Qtd. de Parcelas</label>
                    <input
                      type="number"
                      min={1}
                      max={60}
                      ...
                      className="w-full h-9 px-3 rounded-lg border border-slate-200 bg-white text-xs font-bold text-slate-800 focus:border-[#0052CC] outline-none"
                    />
                  </div>

                  {/* Intervalo em Dias */}
                  <div className="space-y-1">
                    <label className="text-[11px] font-bold text-slate-700">Intervalo (dias)</label>
                    <input
                      type="number"
                      min={1}
                      max={180}
                      value={formIntervaloDias}
                      onChange={(e) => setFormIntervaloDias(Math.max(1, parseInt(e.target.value, 10) || 30))}
                      placeholder="30"
                      className="w-full h-9 px-3 rounded-lg border border-slate-200 bg-white text-xs font-bold text-slate-800 focus:border-[#0052CC] outline-none"
                    />
                  </div>
                ...
                {/* Tabela / Lista de Parcelas Geradas Automaticamente */}
                {calculatedParcelas.length > 0 && (
                  <div className="pt-2 border-t border-slate-200/80 space-y-1.5">
                    <div className="flex items-center justify-between text-[11px] font-bold text-slate-700">
                      <span>Parcelas Calculadas Automaticamente ({calculatedParcelas.length}):</span>
                      {calculatedParcelas[0]?.valor > 0 && (
                        <span className="text-emerald-700 font-bold">
                          {calculatedParcelas.length}x de R$ {calculatedParcelas[0].valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </span>
                      )}
                    </div>
                    <div className="max-h-36 overflow-y-auto space-y-1 custom-scrollbar pr-1">
                      {calculatedParcelas.map((parc, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between px-2.5 py-1.5 rounded-lg bg-white border border-slate-200 text-xs text-slate-700"
                        >
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-[#0052CC] bg-blue-50 px-1.5 py-0.5 rounded text-[10px]">
                              Parcela {parc.numero}
                            </span>
                            <span className="text-[11px] text-slate-500">
                              Vencimento: <strong>{formatDateBR(parc.dataVencimento)}</strong>
                            </span>
                          </div>
