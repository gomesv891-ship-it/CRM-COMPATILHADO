...
export const OrcamentosListScreen: React.FC<OrcamentosListScreenProps> = ({
  ...
  const handleCopyImage = async (orc: SavedOrcamento) => {
    ...
    try {
      setIsExporting(true);
      showToast('Copiando imagem do orçamento...');
      ...
      try {
        ...
        console.warn('html2canvas falhou, tentando toPng...', errCanvas);
      ...
      if (blob && navigator.clipboard && typeof ClipboardItem !== 'undefined') {
        ...
        showToast('✓ Imagem copiada! Cole na conversa do WhatsApp com Ctrl+V.');
      } else {
        showToast('✓ Imagem gerada! Caso seu navegador não permita colar direto, utilize o botão "Baixar Imagem".');
      }
    } catch (err) {
  ...
  // Active orcamentos list considering director responsible tabs vs common user isolation
  const activeOrcamentosSource = useMemo(() => {
    if (isDirector) {
      if (responsibleTab === 'Todos') {
        return orcamentos;
      }
      return orcamentos.filter((o) => isRecordOfResponsible(o, responsibleTab));
    }
    // Usuários comuns visualizam somente seus próprios orçamentos
    return userOrcamentos;
  }, [isDirector, responsibleTab, orcamentos, userOrcamentos]);
  ...
  return (
    <div className="w-full px-4 sm:px-8 xl:px-10 py-6 space-y-5">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
          ...
          <button
            ...
            className="h-10 px-4 sm:px-5 rounded-xl text-white font-semibold text-xs sm:text-sm flex items-center gap-2 shadow-xs transition-colors cursor-pointer bg-[#0066ff] hover:bg-blue-600"
          >
      ...
      {/* Banner de Orçamento em Andamento */}
      {hasInProgressDraft && (
        <div className="bg-gradient-to-r from-blue-50 via-indigo-50/70 to-emerald-50/50 border border-blue-200/90 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
          <div className="flex items-start sm:items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#0066ff] text-white flex items-center justify-center flex-shrink-0 shadow-xs">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-[#0052cc]">
                  Orçamento em Preenchimento Preservado
                </span>
              </div>
              <p className="text-xs sm:text-sm text-slate-700 mt-0.5">
                Você possui um orçamento em andamento{' '}
                {hasInProgressDraft.items?.length
                  ? `com ${hasInProgressDraft.items.length} ${hasInProgressDraft.items.length === 1 ? 'item' : 'itens'}`
                  : ''}
                {hasInProgressDraft.nomeOrcamento ? ` ("${hasInProgressDraft.nomeOrcamento}")` : ''}. Seus dados estão salvos com segurança.
              </p>
            </div>
          </div>
          <button
            onClick={onNewOrcamento}
            className="self-end sm:self-center h-9 px-4 rounded-xl bg-[#0066ff] hover:bg-blue-600 text-white font-semibold text-xs sm:text-sm flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer flex-shrink-0"
          >
      ...
      {isDirector && (
        <ResponsibleFilterTabs
          ...
          label="Orçamentos por Responsável"
        />
      )}
